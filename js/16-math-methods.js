console.clear();

console.log(Math.E);
console.log(Math.PI);
console.log(Math.SQRT2);

console.log(Math.abs(-7));
console.log(Math.abs(7));

console.log(Math.ceil(5));    //apvalina is virsasu
console.log(Math.ceil(5.5));
console.log(Math.ceil(5.1));
console.log(Math.ceil(5.99999));
console.log(Math.ceil(5.000001));

console.log(Math.floor(999));        //apvalina is apacios
console.log(Math.floor(999.99999));
console.log(Math.floor(999.5));
console.log(Math.floor(999.00001));

console.log(Math.round(1.5));   // apvalina mokykliskai
console.log(Math.round(1.0001));
console.log(Math.round(1.9999));
console.log(Math.round(1.4999));

console.log(Math.round(-1.4999));
console.log(Math.round(-1.5));
console.log(Math.round(-1.9999));
console.log(Math.round(-1.50001));


console.log(Math.exp(2, 3));
console.log(Math.max(2, 3, 5, 1, -7));
console.log(Math.min(2, 3, -7, 5, 1, -7));

console.log(Math.pow(2, 3), 2 ** 3);

console.log(Math.sqrt(9));
console.log(Math.sqrt(81));
console.log(Math.sqrt(144));
console.log(Math.sqrt(-1));     // i (menamasis skaicius)